...
class cstruct2:
    def metafield_to_field(self, name: str | None, datatype, data: int | str | tuple, anonymous = False):
        """
        ...
        # bit fields
            ...
            if switch_obj.dependent not in self.field_names:
                raise cstruct2_switch_dependent_wrong(
                    name,
                    switch_obj.dependent
                )
            ...
            for case, corresponding_field in switch_obj.decisions.items():
                new_switch_obj.decisions[case] = self.metafield_to_field(
                    ...
                    (tuple(list(corresponding_field)[1:]) 
                        if len(corresponding_field) > 2 else corresponding_field[1]),
                    anonymous = True
                )
            ...
            field = cstruct2_recursive_wrapper(
                name,
                value
            )
        ...
        if not isinstance(width, str) and not isinstance(width, int):
            if datatype != "cstruct2" and datatype != "switch_type":
                raise AttributeError(f"The width, {width}, on field {name}, is not int or str.")
    ...
    def __init__(self, another_class):
        ...
        self.members = filter(lambda x: not x.startswith("_"), another_class.__dict__.keys())
    ...
    def parse_field(self, stream, field, recursive = False):
        ...
        if not isinstance(field, cstruct2_recursive_wrapper) and not isinstance(field, switch_type) and not isinstance(field, list):
        ...
        if isinstance(field, list):
            ...
            values[field.name] = wrapper(int.from_bytes(
    ...
    def from_stream(self, stream: RawIOBase) -> dict:
        ...
        of it's high-level, processed contents. Basically a for loop over all fields
    ...
    def __ws_value_checker(self, allowed: list, given):
        ...
        raise cstruct2_invalid_value_exception(
            type(given),
            allowed
        )
    ...
    def write_field(self, values: dict, value, field, stream):
        ...
        # together. 
        if (not isinstance(field, list) and not isinstance(field, cstruct2_recursive_wrapper) 
        ...
        if isinstance(field, list):
            ...
            data: bytes = int.to_bytes(value, absolute_width, byteorder = field.endianness)
    ...
    def __len__(self) -> int:
        ...
        # ~~~~~~~~~~~~~^ it's ceilinged because if we only read 4 bits, we've still read a byte.
